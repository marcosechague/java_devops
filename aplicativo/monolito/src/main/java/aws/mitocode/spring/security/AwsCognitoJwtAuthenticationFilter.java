		...
		if (rq.getServletPath().indexOf("/security/login") >= 0
			...
			try {
				...
				if (tokenJWT != null) {
					...
					DecodedJWT jwtDecoded = verificadorJWT.verify(tokenJWT);

					String usuarioCognito = jwtDecoded.getClaims().get(COGNITO_USERNAME).asString();

					if (usuarioCognito != null) {

						List<String> groups = (List<String>) jwtDecoded.getClaims().get(COGNITO_GROUPS).asList(String.class);
